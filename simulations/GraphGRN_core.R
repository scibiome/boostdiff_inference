...
sampleSubNetwork <- function(graph, size, minregs, k, seed) {
  ...
  # Samples the full graph to satisfy
  # specified graph size, Min. no. of TFs
...
getAMC <- function(graph, directed = T) {
  ...
  # from the nodes and edges of the graph
...
randomizeParamsC <- function(graph, type = 'linear', seed) {
  ...
  # Get random parameters for EC50 and n
